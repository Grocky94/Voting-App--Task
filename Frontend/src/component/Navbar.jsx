...
const Navbar = () => {
    ...
    const openLogin = () => {
        setIsShown(true);
        setTimeout(() => {
            setIsShown(false); // Close dropdown after 2 seconds
        }, 2000);
    };
    ...
    const handleVotingbutton = () => {
        if (state?.user?.vote === true) {
            return setMessage("Sorry, you have already voted.");
        } else {
            setMessage('')
        }
    }
        ...
        <>
            <div className="nav">
                ...
                {isShown && (<div className='dropdown' >
                    ...
                    {state?.user?.role === "Admin" ? <div className="dropdown-child" onClick={() => redirect('/adminPage')}>Admin Panel</div> : null}
                    {state?.user?.role === "User" ? <div className="dropdown-child" onClick={() => { redirect('/'), handleVotingbutton() }}>Voting Booth</div> : null}
